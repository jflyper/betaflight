...
void uartReconfigure(uartPort_t *uartPort)
{
    ...
    {
#ifdef USE_DMA
        if (uartPort->rxDMAInstance)
        {
            uartPort->rxDMAHandle.Instance = (DMA_INSTANCE_TYPE *)uartPort->rxDMAInstance;
#if !defined(STM32H7)
    ...
    if (uartPort->port.mode & MODE_TX) {
#ifdef USE_DMA
        if (uartPort->txDMAInstance) {
            uartPort->txDMAHandle.Instance = (DMA_INSTANCE_TYPE *)uartPort->txDMAInstance;
#if !defined(STM32H7)
