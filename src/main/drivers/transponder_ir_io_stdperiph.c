...
static dmaInstance_t *dmaRef = NULL;
...
void transponderIrHardwareInit(ioTag_t ioTag, transponder_t *transponder)
{
    ...
    DMA_Init((DMA_INSTANCE_TYPE *)dmaRef, &DMA_InitStructure);
...
{
    DMA_SetCurrDataCounter((DMA_INSTANCE_TYPE *)dmaRef, transponder->dma_buffer_size);  // load number of bytes to be transferred
    TIM_SetCounter(timer, 0);
    TIM_Cmd(timer, ENABLE);
    DMA_Cmd((DMA_INSTANCE_TYPE *)dmaRef, ENABLE);
}
