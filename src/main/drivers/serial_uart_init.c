...
serialPort_t *uartOpen(UARTDevice_e device, serialReceiveCallbackPtr rxCallback, void *rxCallbackData, uint32_t baudRate, portMode_e mode, portOptions_e options)
{
    ...
    if (mode & MODE_RX) {
        ...
        if (s->rxDMAInstance) {
            ...
            DMA_DeInit((DMA_INSTANCE_TYPE *)s->rxDMAInstance);
            DMA_Init((DMA_INSTANCE_TYPE *)s->rxDMAInstance, &DMA_InitStructure);
            DMA_Cmd((DMA_INSTANCE_TYPE *)s->rxDMAInstance, ENABLE);
            USART_DMACmd(s->USARTx, USART_DMAReq_Rx, ENABLE);
            s->rxDMAPos = DMA_GetCurrDataCounter((DMA_INSTANCE_TYPE *)s->rxDMAInstance);
#else
    ...
    if (mode & MODE_TX) {
        ...
        if (s->txDMAInstance) {
            ...
            DMA_DeInit((DMA_INSTANCE_TYPE *)s->txDMAInstance);
            DMA_Init((DMA_INSTANCE_TYPE *)s->txDMAInstance, &DMA_InitStructure);
            DMA_ITConfig((DMA_INSTANCE_TYPE *)s->txDMAInstance, DMA_IT_TC | DMA_IT_FE | DMA_IT_TE | DMA_IT_DME, ENABLE);
            DMA_SetCurrDataCounter((DMA_INSTANCE_TYPE *)s->txDMAInstance, 0);
#else
            DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralDST;
            DMA_InitStructure.DMA_Mode = DMA_Mode_Normal;
            DMA_DeInit((DMA_INSTANCE_TYPE *)s->txDMAInstance);
            DMA_Init((DMA_INSTANCE_TYPE *)s->txDMAInstance, &DMA_InitStructure);
            DMA_ITConfig((DMA_INSTANCE_TYPE *)s->txDMAInstance, DMA_IT_TC, ENABLE);
            DMA_SetCurrDataCounter((DMA_INSTANCE_TYPE *)s->txDMAInstance, 0);
#endif
