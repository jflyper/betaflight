...
uint32_t dmaFlag_IT_TCIF(const dmaInstance_t *stream)
{
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 0);
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 1);
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 2);
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 3);
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 4);
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 5);
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 6);
    RETURN_TCIF_FLAG((DMA_INSTANCE_TYPE *)stream, 7);
    return 0;
}
...
dmaIdentifier_e dmaGetIdentifier(const dmaInstance_t* instance)
{
...
dmaChannelDescriptor_t* dmaGetDescriptor(const dmaInstance_t* instance)
{
    for (int i = 0; i < DMA_LAST_HANDLER; i++) {
        if (dmaDescriptors[i].ref == instance) {
