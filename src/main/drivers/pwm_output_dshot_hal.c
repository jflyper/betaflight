...
static void processInputIrq(motorDmaOutput_t * const motor)
{
    ...
    if (useBurstDshot) {
        LL_EX_DMA_DisableStream((DMA_INSTANCE_TYPE *)motor->timerHardware->dmaTimUPRef);
        LL_TIM_DisableDMAReq_UPDATE(motor->timerHardware->tim);
    } else
#endif
    {
        LL_EX_DMA_DisableStream((DMA_INSTANCE_TYPE *)motor->dmaRef);
...
)
{
    ...
    LL_EX_DMA_DeInit((DMA_INSTANCE_TYPE *)motor->dmaRef);
    ...
    LL_EX_DMA_Init((DMA_INSTANCE_TYPE *)motor->dmaRef, pDmaInit);
    LL_EX_DMA_EnableIT_TC((DMA_INSTANCE_TYPE *)motor->dmaRef);
}
...
#ifdef USE_DSHOT_DMAR
        if (useBurstDshot) {
            LL_EX_DMA_SetDataLength((DMA_INSTANCE_TYPE *)dmaMotorTimers[i].dmaBurstRef, dmaMotorTimers[i].dmaBurstLength);
            LL_EX_DMA_EnableStream((DMA_INSTANCE_TYPE *)dmaMotorTimers[i].dmaBurstRef);
    ...
    if (DMA_GET_FLAG_STATUS(descriptor, DMA_IT_TCIF)) {
...
#endif
        {
            ...
            if (useDshotTelemetry) {
                ...
                LL_EX_DMA_SetDataLength((DMA_INSTANCE_TYPE *)motor->dmaRef, motor->dmaInputLen);
...
#define DMAINIT dmaInitStruct
    ...
    LL_EX_DMA_DisableStream((DMA_INSTANCE_TYPE *)dmaRef);
    LL_EX_DMA_DeInit((DMA_INSTANCE_TYPE *)dmaRef);
    ...
    LL_EX_DMA_Init((DMA_INSTANCE_TYPE *)dmaRef, &DMAINIT);
    LL_EX_DMA_EnableIT_TC((DMA_INSTANCE_TYPE *)dmaRef);
    motor->dmaRef = dmaRef;
