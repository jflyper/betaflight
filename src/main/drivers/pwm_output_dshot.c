...
static void processInputIrq(motorDmaOutput_t * const motor)
{
    ...
    DMA_Cmd((DMA_INSTANCE_TYPE *)motor->dmaRef, DISABLE);
...
)
{
    ...
    dmaInstance_t *dmaRef = motor->dmaRef;
    ...
    DMA_DeInit((DMA_INSTANCE_TYPE *)dmaRef);
    ...
    DMA_Init((DMA_INSTANCE_TYPE *)dmaRef, pDmaInit);
    DMA_ITConfig((DMA_INSTANCE_TYPE *)dmaRef, DMA_IT_TC, ENABLE);
}
...
#ifdef USE_DSHOT_DMAR
        if (useBurstDshot) {
            DMA_SetCurrDataCounter((DMA_INSTANCE_TYPE *)dmaMotorTimers[i].dmaBurstRef, dmaMotorTimers[i].dmaBurstLength);
            DMA_Cmd((DMA_INSTANCE_TYPE *)dmaMotorTimers[i].dmaBurstRef, ENABLE);
    ...
    if (DMA_GET_FLAG_STATUS(descriptor, DMA_IT_TCIF)) {
...
#endif
        {
#ifdef USE_DSHOT_DMAR
            if (useBurstDshot) {
                DMA_Cmd((DMA_INSTANCE_TYPE *)motor->timerHardware->dmaTimUPRef, DISABLE);
            ...
            if (useDshotTelemetry) {
                ...
                DMA_SetCurrDataCounter((DMA_INSTANCE_TYPE *)motor->dmaRef, motor->dmaInputLen);
...
#define DMAINIT dmaInitStruct
    ...
    DMA_Cmd((DMA_INSTANCE_TYPE *)dmaRef, DISABLE);
    DMA_DeInit((DMA_INSTANCE_TYPE *)dmaRef);
