...
const uartHardware_t uartHardware[UARTDEV_COUNT] = {
#ifdef USE_UART1
    {
        .device = UARTDEV_1,
        .reg = USART1,
        .rxDMAInstance = UART1_RX_DMA,
        .txDMAInstance = UART1_TX_DMA,
...
#ifdef USE_UART2
    {
        .device = UARTDEV_2,
        .reg = USART2,
        .rxDMAInstance = UART2_RX_DMA,
        .txDMAInstance = UART2_TX_DMA,
...
#ifdef USE_UART3
    {
        .device = UARTDEV_3,
        .reg = USART3,
        .rxDMAInstance = UART3_RX_DMA,
        .txDMAInstance = UART3_TX_DMA,
    ...
    // UART4 XXX Not tested (yet!?) Need 303RC, e.g. LUX for testing
    {
        .device = UARTDEV_4,
        .reg = UART4,
        .rxDMAInstance = 0, // XXX UART4_RX_DMA !?
        .txDMAInstance = 0, // XXX UART4_TX_DMA !?
...
static void handleUsartTxDma(dmaChannelDescriptor_t* descriptor)
{
    ...
    DMA_Cmd((DMA_INSTANCE_TYPE *)descriptor->ref, DISABLE);
...
uartPort_t *serialUART(UARTDevice_e device, uint32_t baudRate, portMode_e mode, portOptions_e options)
{
    ...
    if (!s->rxDMAInstance || !s->txDMAInstance) {
...
void uartIrqHandler(uartPort_t *s)
{
    ...
    if (!s->rxDMAInstance && (ISR & USART_FLAG_RXNE)) {
    ...
    if (!s->txDMAInstance && (ISR & USART_FLAG_TXE)) {
