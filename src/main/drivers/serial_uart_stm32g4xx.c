...
#define UART_BUFFERS(n) \
  DMA_RAM_R volatile uint8_t uart ## n ## RxBuffer[UART_RX_BUFFER_SIZE]; \
  DMA_RAM_W volatile uint8_t uart ## n ## TxBuffer[UART_TX_BUFFER_SIZE]; struct dummy_s
...
const uartHardware_t uartHardware[UARTDEV_COUNT] = {
    ...
    // On G474, UART6 is implemented as LPUART1
    {
        ...
        .reg = LPUART1,
#ifdef USE_DMA
        .rxDMARequest = DMA_REQUEST_LPUART1_RX,
        .rxDMAResource = (dmaResource_t *)UART6_RX_DMA_CHANNEL,
        .txDMARequest = DMA_REQUEST_LPUART1_TX,
...
void uartIrqHandler(uartPort_t *s)
{
    ...
    /* UART in interrupting transmitter -----------------------------------------*/
    if (
#ifdef USE_DMA
        !s->txDMAResource &&
#endif
