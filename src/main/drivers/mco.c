...
void mcoInit(const mcoConfig_t *mcoConfig)
{
    ...
    // G4 only supports one MCO
    if (mcoConfig->enabled) {
        IO_t io = IOGetByTag(DEFIO_TAG_E(PA8));
        IOInit(io, OWNER_MCO, 1);
        HAL_RCC_MCOConfig(RCC_MCO, mcoSources[mcoConfig->source], mcoDividers[mcoConfig->divider]);
    }
#else
